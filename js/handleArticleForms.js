 ...
 */
function processArtEditFrmData(event,articleId,offset, totalCount, serverUrl, method, current){
    ...
    if(method==='POST'){
        ...
        .finally(() => window.location.hash=`#articles/${current}/${offset}`);
    }else{
    ...
    fetch(`${serverUrl}/article/${articleId}`, postReqSettings)  //now we need the second parameter, an object wih settings of the request.
        ...
        .finally(() => window.location.hash=`#article/${articleId}`);
    }
